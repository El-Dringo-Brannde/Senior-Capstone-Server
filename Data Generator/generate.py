	...
	for i in range(len(avail_states)):
		...
		for j in range(num_cities):
			...
			for k in range(num_dealerships):
				...
				shift_lat = float(np.random.normal(0, 5.0, 1) / converstion_factor)
				shift_long = float(np.random.normal(0, 5.0, 1) / converstion_factor)

				lat = (((float(locations[loc]["lat"]) + shift_lat)/40075) * 360)
				lon = (((float(locations[loc]["long"]) + shift_long)/40075) * 360)
...
def load_options():
	...
	if(get_choice("string", ["y","n"], prompt, "y") == "n"):
		...
		if(get_choice("string", ["y", "n"], prompt, "y") == "y"):
			...
			cities_max = get_choice("int", [cities_min, 1000], prompt, cities_max)
		else:
			prompt = "Number of cities to generate per state [" + str(cities_max) + "]: "
		...
		if(get_choice("string", ["y", "n"], prompt, "y") == "y"):
			prompt = "Minimum number of dealerships [" + str(dealerships_min) + "]: "
			dealerships_min = get_choice("int", [1, 1000], prompt, dealerships_min)

			prompt = "Max number of dealerships [" + str(dealerships_max) + "]: "
			dealerships_max = get_choice("int", [dealerships_min, 1000], prompt, dealerships_max)
		else:
			prompt = "Number of dealerships to generate per city [" + str(dealerships_max) +"]: "
			dealerships_min = get_choice("int", [1, 1000], prompt, dealerships_max)
